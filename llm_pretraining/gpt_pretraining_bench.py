...
def flops_per_token_training(gpt_cfg, num_params: int):
    ...
    Aproximación estándar para entrenamiento (forward + backward pass):
    phi ≈ 6N (forward) * 3 (training overhead) = 18N
    
    Donde N = número de parámetros del modelo
    ...
    """
    N = int(num_params)
    # Factor 3 para forward + backward + optimizer step overhead
    return 3 * 6 * N
...
def estimate_mfu(tokens_per_sec, gpt_cfg, num_params, peak_flops=GPU_FP32_PEAK_FLOPS):
    ...
    Para entrenamiento moderno usa Tensor FLOPs (mixed precision) por defecto
    """
